...
public class MainSurface extends SurfaceView implements SurfaceHolder.Callback {
    ...
    public boolean onTouchEvent(MotionEvent event) {
        if (event.getAction() == MotionEvent.ACTION_UP) {
            ...
            if (pressTime >= 250) {
                if (event.getX() >= 0 && event.getX() <= this.tetrisTileSize && event.getY() >= 0 && event.getY() <= this.tetrisTileSize) {
                    showHiddenInfos = !showHiddenInfos;
                    this.settingStorage.set(1, showHiddenInfos ? "true" : "false");
                } else if (event.getX() >= ((this.tetrisControl.TETRIS_WIDTH-1) * this.tetrisTileSize) && event.getX() <= (this.tetrisControl.TETRIS_WIDTH * this.tetrisTileSize) && event.getY() >= 0 && event.getY() <= this.tetrisTileSize) {
                    this.tetrisDraw.detailDrawMode = !this.tetrisDraw.detailDrawMode;
                    this.settingStorage.set(2, this.tetrisDraw.detailDrawMode  ? "true" : "false");
                } else {
                    showScore = !showScore;
    ...
    private void resetTetris() {
        ...
        if(!this.mediaPlayer.isPlaying())
        {
            ...
            this.mediaPlayer = MediaPlayer.create(this.getContext(), R.raw.tetris);
            this.mediaPlayer.setLooping(true);
            this.mediaPlayer.start();
        }
    ...
    public void draw(Canvas canvas) {
        ...
        if (!this.tetrisRunning) {
            if (this.showGameOver) {
                ...
                this.tetrisDraw.drawCenterXText(canvas, "Score " + Integer.toString(this.lastScore), -240, paint);
            ...
            this.tetrisDraw.drawActionButtons(canvas, TetrisClickAction.SECTION_HEIGHT,this.getContext());
            this.lastScore = this.tetrisControl.getScore();
            if (showScore && !showHiddenInfos) {
                this.tetrisDraw.drawScore(canvas, this.lastScore);
            } else if (showHiddenInfos) {
                this.tetrisDraw.drawHiddenInfos(canvas, GameThread.getFPS(), canvas.getWidth(), canvas.getHeight(), this.tetrisTileSize, this.tetrisTime, this.gameTime, this.lastScore, this.tetrisControl.tetrisSpeed);
            }
        }
    }
